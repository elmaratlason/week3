...
INSTANCE_ID=$(cat ec2_instance-jenkins2/instance-id.txt)

# check if the instance id is our
if [ $INSTANCE_ID == $(cat ec2_instance-jenkins2/instance-id.txt) ]
then
  INSTANCENAME=$(cat ec2_instance-jenkins2/instance-public-name.txt)
else
  echo "invalid instance id"
  echo "quitting..."
  echo "my id is: $(cat ec2_instance-jenkins2/instance-id.txt)"
  exit 1
fi

# awscomputer
REMOTE_COMPUTER=ec2-user@$INSTANCENAME
PEM_FILE=$(ls ec2_instance-jenkins2/*.pem)
